...
核心做法:把预先采集的真人鼠标轨迹随机加权混合,再按目标方向拉伸,生成每次都不同、视觉上接近真人的移动路径。
...
本项目换个思路:**不从零合成,直接复用真人画出来的轨迹。** 仓库附带 402 条真人轨迹样本(每条 19 个采样点)。

## 原理

![原理示意图](images/principle.png)
...
普通模式下,鼠标从起点沿一条混合轨迹直接走向目标,接近目标时减速收尾。同样起止点各运行 5 次,移动中实时采样真实光标位置;左为贝塞尔,右为本方法:

![左下到右上对比](images/traj_bl_to_tr.png)

![右下到左上对比](images/traj_br_to_tl.png)

可以看出:贝塞尔(左)平滑得像圆规画的;本方法(右)带有真人数据特有的不规则微抖与加减速。

### 迭代 / 漫游模式

开启 `IterMode` 后,长距离移动会递归填充中间段。填充用的也是真实片段,偶尔抽到波动大的片段会把光标带向较远方向,形成"中途游走、最后回到目标"的效果。仅本方法,左下→右上,3 条:

![迭代漫游模式](images/traj_iter_bl_to_tr.png)
...
![距离-耗时对比](images/timing_distance.png)
...
3. **样本来自单人采集。** 所有输出带有同一个人的运动风格,多账号 / 大规模场景下可能成为可识别特征。如需更高多样性,自行采集多人数据。
4. **漫游模式没有意图。** 中途游走由数据波动驱动,可能移向屏幕空白处,严格分析下反而可疑。
5. **执行层基于 pyautogui。** 合成事件在某些检测视角下可被识别为非真实输入。

## 采集自己的数据

样本是一个 CSV,每行为一条归一化真人轨迹(相对位移序列 + 总耗时 + 终点)。可用附带脚本录制自己的数据替换它——**多采几个人,多样性和拟真度都会更好。**
